...
public:
	 ...
	 *///train(training_landmarks, x0, Mat(), hog, print_residual)
	template<class ProjectionFunction, class OnTrainingEpochCallback>//ProjectionFunction = HogTransform
	...
	{
		using cv::Mat;
		Mat current_x = initialisations;//注意 初始化只有五个关键点
		for (size_t regressor_level = 0; regressor_level < regressors.size(); ++regressor_level) {//回归器数量（级联回归）
			...
			//typename std::result_of<ProjectionFunction(Mat, size_t, int)>::type>自动推导出ProjectionFunction的模板函数（）的返回值类型
			...
			for (int sample_index = 0; sample_index < current_x.rows; ++sample_index) {//遍历样本
				cv::Mat update_step = current_x.row(sample_index) - parameters.row(sample_index);//计算偏移量
